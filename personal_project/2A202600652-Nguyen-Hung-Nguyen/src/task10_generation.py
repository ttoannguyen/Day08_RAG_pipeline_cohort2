...
def format_context(chunks: list[dict]) -> str:
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        doc_type = chunk.get("metadata", {}).get("doc_type", chunk.get("metadata", {}).get("type", "unknown"))
...
def generate_with_citation(query: str, top_k: int = TOP_K) -> dict:
    ...
    if not api_key or api_key.startswith("sk-xxx") or api_key == "":
    ...
    response = client.chat.completions.create(
        ...
        extra_headers={
            ...
            "X-Title": "Antigravity RAG Pipeline"
        }
    )
